...
import {
  ...
  SENTIMENT_CONFIG,
  CATEGORY_LABELS,
  SURVEY_COMPLETION_TREND,
  type TeamMember,
} from "@/lib/mockData";
import { LineChart as LineIcon, ChevronDown, ChevronUp, Brain, ClipboardList, Users } from "lucide-react";
      ...
      {open && (
        <div className="px-5 pb-5 border-t" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          <div className="pt-4 grid grid-cols-3 gap-4 mb-4">
            ...
            <div
              className="rounded-xl p-3 text-center"
              style={{ background: `${sentiment.color}0A`, border: `1px solid ${sentiment.color}20` }}
            >
          ...
          >
            ...
            <p className="text-sm text-slate-300 leading-relaxed">{SCORE_INSIGHTS[m.sentiment]}</p>
          </div>
        </div>
      )}
    </div>
...
export default function TeamInsights() {
  ...
  const byScore = [...TEAM_MEMBERS].sort((a, b) => b.score - a.score);
    ...
    <div className="animate-fade-in">
      ...
      <div className="card-purple p-5 mb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="badge badge-green ml-auto">↑ Improving</span>
        </div>
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={SURVEY_COMPLETION_TREND} margin={{ top: 0, right: 10, left: -20, bottom: 0 }}>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {(["proactive", "reactive", "blocked", "neutral"] as const).map((s) => {
          ...
          const count = TEAM_MEMBERS.filter((m) => m.sentiment === s).length;
          return (
      ...
      <div>
        ...
        <div className="space-y-3">
          {TEAM_MEMBERS.map((m) => <MemberCard key={m.id} m={m} />)}
        </div>
      </div>
    </div>
  );
